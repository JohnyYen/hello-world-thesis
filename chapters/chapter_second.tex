...
El diagrama de arquitectura refleja cómo se organiza internamente el módulo Estudiante en tres capas lógicas:
...
Se eligió el patrón MVC para el módulo Estudiante por las siguientes razones:
...
El módulo Profesor fue diseñado bajo una arquitectura cliente-servidor, ideal para sistemas web donde se requiere comunicación remota, escalabilidad moderada y separación clara entre interfaz y lógica de negocio. Este patrón permite que el sistema sea fácil de mantener, testeable y preparado para futuras evoluciones técnicas.
...
\begin{description}
	...
	\item[Lógica de Negocio (Servidor):] Implementa funcionalidades clave como el registro de progreso estudiantil, generación de gráficos comparativos, asignación de niveles especiales a estudiantes específicos, entre otras. Esta capa se organiza en paquetes según su responsabilidad, promoviendo modularidad y claridad en el código.
\end{description}
...
El \textbf{Backend} constituye el núcleo de la plataforma, encargada de guardar el progreso obtenido por parte de los estudiantes en el videojuego y devolvérselo al frontend del profesor. Para su implementación se utilizó una arquitectura orientada a servicios, organizado en módulos separados como gestión de usuarios, de niveles o de progreso, permitiendo así escalar a nuevos futuros módulos si la aplicación lo requiere. Para exponer estos servicios al frontend se utilizó el estandar API RESTful, basado en los principios de \textit{Representational State Transfer} (REST), propuesto por Roy Fielding en el año 2000. Este estándar define un conjunto de restricciones para la construcción de servicios web, entre las cuales destacan: la comunicación sin estado (\textit{stateless}), la manipulación de recursos mediante identificadores uniformes (URI), el uso de métodos estándar del protocolo HTTP (GET, POST, PUT, DELETE, entre otros) y la representación de datos en formatos ligeros como JSON o XML.
...
Este patrón asegura que una clase tenga una única instancia global accesible. El \texttt{GameManager} se implementa como un Singleton, centralizando la gestión del flujo del juego y evitando la creación de múltiples instancias que podrían generar inconsistencias en el estado global.
