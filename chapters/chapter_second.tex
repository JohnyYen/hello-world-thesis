...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figuras/c4_componentes_estudiante.png}
...
El módulo del profesor se implementa como una aplicación web basada en arquitectura cliente–servidor. Su diagrama de componentes muestra los elementos principales que gestionan la interfaz docente y la comunicación con la API backend.

\begin{itemize}
    \item \textbf{Dashboard}: Componente principal que muestra estadísticas globales y comparativas del grupo.
    \item \textbf{LevelEditor}: Permite la creación y modificación de niveles educativos mediante una interfaz visual.
    \item \textbf{StudentTracker}: Gestiona los datos individuales y grupales de los estudiantes.
    \item \textbf{ReportGenerator}: Genera informes y gráficos de desempeño.
    \item \textbf{APIClient}: Componente encargado de la comunicación con la API REST para sincronizar datos.
\end{itemize}
...
El backend constituye el núcleo de la plataforma. Su diagrama de componentes refleja la estructura interna del servicio, organizado según los principios de separación de responsabilidades y escalabilidad.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figuras/c4_componentes_backend.png}
