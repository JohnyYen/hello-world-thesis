...
El presente capítulo establece las bases conceptuales y teóricas que sustentan la investigación. Se analizan los videojuegos como fenómeno cultural y tecnológico, definiendo sus características esenciales y las principales taxonomías, con especial atención a los \textit{serious games} y a la novela visual como género base para el diseño de la solución propuesta. A continuación, se examina el potencial pedagógico de los videojuegos y su aplicación específica a la enseñanza de la programación, subrayando ventajas como la materialización de conceptos abstractos, la reducción de la carga emocional del error y la regulación del ritmo de aprendizaje. Estos fundamentos se complementan con el estudio de las teorías que respaldan su uso, las cuales proporcionan los criterios de diseño que orientan la construcción de experiencias educativas basadas en juegos.

El capítulo continúa con el análisis de la gamificación, desglosando sus elementos y la dinámica de motivación que promueve en el aula. Se dedica una sección a los videojuegos educativos adaptativos, donde se presentan sus dimensiones lúdica y pedagógica, el modelo del estudiante como núcleo del sistema y las principales estrategias de personalización. Posteriormente, se realiza un análisis de videojuegos educativos representativos empleados para la enseñanza de la programación, identificando sus aportes y limitaciones. Por último, se revisan los tableros de analíticas de aprendizaje en entornos de videojuegos, sus principios de diseño y ejemplos concretos, cerrando con las conclusiones parciales que sintetizan el marco teórico y justifican el diseño de la plataforma propuesta en los capítulos siguientes.
...
\textbf{taxonomía} o clasificación (ver Figura \ref{fig:taxonomy}), ya que permite comprender su diversidad
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth, height=0.5\textheight, keepaspectratio]{src/doc/taxonomia_games.png}
	\caption[Taxonomia de la clasificación y géneros de los videojuegos]{Taxonomia de la clasificación y géneros de los videojuegos}
	\label{fig:taxonomy}
\end{figure}
...
Los fundamentos de la gamificación se estructuran en tres niveles jerárquicos (Dinámicas, Mecánicas y Componentes) propuestos por Werbach y Hunter \cite{ortiz-colon_gamificacion_2018,morales_elearning_noda}.  
Estos niveles permiten comprender cómo se construyen las experiencias gamificadas y cómo cada elemento contribuye al compromiso, la motivación y el aprendizaje del usuario (ver Tabla \ref{tab:gamification-components}).
...
\begin{table}[H]
	\centering
	\label{tab:gamification-components}
...
dos dimensiones interdependientes. La primera es la adaptación lúdica,
...
la adaptación pedagógica, que ajusta el contenido educativo en sí mismo:
...
que se requiera en las decisiones del sistema \cite{shum_personalised_2023}.

El enfoque más simple es la adaptación basada en reglas heurísticas.
...
durante el diseño \cite{shum_personalised_2023}.

Un segundo enfoque es la adaptación basada en modelos probabilísticos,
...
El tercer enfoque corresponde a la adaptación basada en aprendizaje
...
El ciclo se compone de cuatro etapas \cite{shum_personalised_2023}:
...
\begin{enumerate}
	...
	registra como un evento estructurado.
	
	\item Actualización del modelo: los nuevos eventos se incorporan al modelo del estudiante, modificando las estimaciones sobre su nivel de dominio, su estilo de aprendizaje o su estado motivacional
	
	\item Decisión: el agente de adaptación
	consulta el modelo actualizado y, aplicando la estrategia para la que fue diseñado,
	determina si es necesario intervenir y en qué dirección
	
	\item Intervención: la decisión se materializa en el videojuego —se
...
El análisis de las soluciones existentes permite identificar tanto los aportes como las limitaciones de los videojuegos actuales orientadas al aprendizaje de la programación. A continuación se examinan cuatro referencias representativas del estado del arte, cuyos rasgos distintivos se sintetizan en la Tabla \ref{tab:comparacion_videojuegos}.
...
\begin{table}[H]
	...
	\caption{Comparación de videojuegos educativos según criterios pedagógicos y de gestión docente.}
	\label{tab:comparacion_videojuegos}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}@{}}
			\toprule
			\textbf{Videojuego}
			& \textbf{Prog. visual bloques}
			& \textbf{Progresión niveles}
			& \textbf{Retroalimentación inmediata}
			& \textbf{Adaptación dificultad}
			& \textbf{Narrativa contextualizada}
			& \textbf{Modelo estudiante}
			& \textbf{Monitoreo docente}
			& \textbf{Tablero analíticas}
			& \textbf{Exportación datos}
			& \textbf{Trazabilidad interacciones}
			& \textbf{Integración LMS} \\
			\midrule
			\textbf{Human Resource Machine} & No      & Sí      & Sí      & No      & Parcial & No      & No      & No      & No      & No      & No      \\
			\midrule
			\textbf{CodeCombat}             & No      & Sí      & Sí      & No      & Parcial & No      & No      & No      & No      & Escasa  & Parcial \\
			\midrule
			\textbf{Scratch}                & Sí      & No      & Escasa  & No      & No      & No      & No      & No      & No      & No      & No      \\
			\midrule
			\textbf{Minerva}                & Sí      & Sí      & Sí      & Sí      & Sí      & Sí      & No      & No      & No      & Parcial & No      \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
...
El análisis de casos concretos permite ilustrar cómo distintas plataformas han abordado la visualización del aprendizaje y la intervención docente, evidenciando tanto sus fortalezas como sus carencias. A continuación se examinan tres referencias representativas, cuyos rasgos se sintetizan en la Tabla \ref{tab:limitaciones_lad}.
...
Kahoot! es una de las plataformas de gamificación más reconocidas en el ámbito educativo. Funciona como un \textit{Student Response System} (SRS) que permite a los docentes diseñar cuestionarios, encuestas y actividades de discusión para evaluar el conocimiento y el compromiso de los estudiantes en tiempo real. Su diseño lúdico incorpora puntuación, música dinámica, colores llamativos, límites de tiempo y tablas de clasificación, transformando el aula en un entorno competitivo similar a un concurso donde los estudiantes compiten activamente por responder con rapidez \cite{singh2024exploring}.

Estudios destacan que esta herramienta potencia la motivación intrínseca, incrementa la satisfacción durante el aprendizaje y promueve un enfoque activo y centrado en el estudiante, permitiendo reforzar contenidos de manera dinámica \cite{singh2024exploring}. Sin embargo, desde la perspectiva de las analíticas, sus métricas se centran en aspectos extrínsecos (como preguntas acertadas y velocidad de respuesta) lo que, si se exagera el componente competitivo, puede derivar en un compromiso superficial y desplazar el foco del aprendizaje profundo hacia la simple obtención de puntos. Asimismo, su capacidad de intervención proactiva para el docente es limitada, restringiéndose principalmente a una retroalimentación reactiva al concluir la actividad \cite{singh2024exploring}.
...
Physics Playground es un videojuego educativo dirigido a estudiantes de secundaria para el aprendizaje de la física newtoniana, cuyo objetivo en cada nivel es guiar una bola verde hasta un globo rojo. El juego ofrece dos modalidades de interacción:

\begin{itemize}
	
	\item \textbf{Niveles de dibujo:} el estudiante traza artilugios sencillos (rampas, palancas, trampolines) sobre la pantalla para crear un camino físico viable.
	
	\item \textbf{Niveles de manipulación:} el alumno ajusta controles deslizantes (\textit{sliders}) para modificar parámetros como la gravedad, la elasticidad o el peso de la bola, o para aplicar fuerzas externas que permitan alcanzar la meta.
	
\end{itemize}

El juego emplea un sistema de evaluación sigilosa (El estudiante es evaluado) basado en el diseño centrado en las evidencias, donde cada acción del jugador funciona como indicador de su comprensión. Mediante un modelo de redes bayesianas, el sistema realiza inferencias en tiempo real sobre nueve competencias clave de física, todo ello sin interrumpir el estado de flujo del estudiante \cite{shute_stealth_2011, mislevy_evidence_2003}.

Fundamentalmente, incorpora un tablero de analíticas llamado \textit{My Backpack}, diseñado bajo un enfoque de maestría intrapersonal que motiva al alumno a compararse consigo mismo y con sus propias metas, en lugar de competir con otros. Este tablero presenta barras de progreso sobre niveles resueltos y monedas recolectadas, junto con gráficos que ilustran el nivel de dominio en cada concepto. En conjunto, este caso ejemplifica una buena práctica en el diseño de tableros: modelos estadísticos robustos operan en segundo plano, mientras que la interfaz visible promueve la reflexión y la autorregulación del aprendizaje.

\textbf{RAGE Analytics}

%RAGE Analytics es un sistema de analíticas de aprendizaje para juegos serios, desarrollado por el grupo de investigación eUCM de la Universidad Complutense de Madrid en el marco de los proyectos europeos H2020 RAGE y BEACONING. Su propósito central es sistematizar el proceso de Game Learning Analytics (GLA), entendido como la aplicación de técnicas de analíticas al contexto específico de los videojuegos educativos, para medir y mejorar el aprendizaje de los estudiantes de forma reproducible y basada en estándares abiertos \cite{rageanalitycs}.
%
%Desde el punto de vista técnico, el sistema adopta la \textit{Experience API} (xAPI) como formato estándar de intercambio de trazas entre el juego y la plataforma de analíticas. Cada interacción del jugador se representa mediante un \textit{statement} xAPI compuesto por tres atributos: un actor, un verbo y un objeto, que en conjunto describen quién realizó qué acción y sobre qué elemento del juego. El modelo de interacción propuesto extiende este formato con conceptos propios del dominio de los juegos serios, tales como \textit{completables} (tareas con inicio y fin), \textit{alternativas} y \textit{variables generales}, permitiendo modelar la práctica totalidad de las interacciones posibles en un juego educativo \cite{rageanalitycs}.
%
%El sistema cuenta con tableros de analíticas diferenciados según el tipo de \textit{stakeholder}: docentes, estudiantes, desarrolladores y gestores. Para los docentes, los tableros ofrecen tanto una vista global del grupo como vistas individuales por estudiante, incluyendo el progreso a lo largo del tiempo, la tasa de aciertos y errores en preguntas de selección múltiple, y el avance en los distintos \textit{completables} del juego. Para los desarrolladores, se proporcionan visualizaciones sobre el uso de verbos xAPI, el acceso a zonas del juego y el comportamiento de los vídeos integrados. Además, el sistema soporta la configuración de alertas y advertencias en tiempo casi real: por ejemplo, notificar al docente cuando un estudiante ha permanecido inactivo durante dos minutos o ha respondido incorrectamente una pregunta de alta relevancia \cite{rageanalitycs}.
%
%Desde la perspectiva del diseño de LAD, este sistema representa una propuesta madura en varios aspectos. En primer lugar, su arquitectura basada en xAPI resuelve el problema de la fragmentación tecnológica, ya que los componentes de seguimiento son reutilizables entre distintos juegos y motores (Unity, Java, C\#). En segundo lugar, la separación entre análisis en tiempo real y análisis por lotes (\textit{batch analysis}) permite tanto la intervención proactiva del docente durante la sesión como la evaluación posterior al juego. No obstante, una limitación identificada es que la personalización avanzada de los tableros requiere el uso de Kibana, herramienta que puede resultar compleja para docentes sin formación técnica, aspecto que los propios autores reconocen como línea de trabajo futura \cite{rageanalitycs}.

RAGE Analytics es un sistema de analíticas de aprendizaje para juegos serios con el objetivo de sistematizar las analíticas de aprendizaje con juegos mediante estándares abiertos que permitan medir y mejorar el aprendizaje de forma reproducible. Técnicamente, adopta el estandar xAPI para representar cada interacción del jugador mediante sentencias compuestos por actor, verbo y objeto, extendiendo este formato con conceptos propios del dominio lúdico como completables, alternativas y variables generales \cite{rageanalitycs}.

El sistema ofrece tableros diferenciados según el perfil de usuario. Los docentes acceden a vistas globales e individuales que muestran el progreso temporal, la tasa de aciertos y errores, y el avance en los completables; los desarrolladores visualizan métricas de uso de verbos xAPI, acceso a zonas del juego y comportamiento de vídeos integrados. Además, permite configurar alertas en tiempo casi real (por ejemplo, ante inactividad prolongada o respuestas incorrectas en preguntas clave) favoreciendo una intervención proactiva del docente \cite{rageanalitycs}.

Desde el diseño del LAD, el sistema se destaca por una arquitectura basada en xAPI que resuelve la fragmentación tecnológica al ser reusable entre distintos motores de videojuegos, y por separar el análisis en tiempo real del análisis por lotes, combinando intervención durante la sesión con evaluación posterior. No obstante, su principal limitación radica en que la personalización avanzada de tableros depende de Kibana, una herramienta que puede resultar compleja para docentes sin formación técnica y que además la sincronización es en tiempo real sin soporte  \textit{offline} \cite{rageanalitycs}.
...
%	\setlength{\tabcolsep}{3pt}
%	\renewcommand{\arraystretch}{0.92}
%	\caption{Principales limitaciones identificadas en los tableros de analíticas de aprendizaje analizados.}
%	\label{tab:limitaciones_lad}
...
%		\toprule
%		\textbf{Limitación} & \textbf{Kahoot!} & \textbf{Duolingo} & \textbf{Physics Playground} & \textbf{RAGE Analytics} \\
%		\midrule
%		Métricas centradas en aprendizaje profundo      & No      & Parcial & Sí      & Sí      \\
%		\midrule
%		Intervención proactiva del docente              & No      & No      & No      & Sí      \\
%		\midrule
%		Vista individualizada por estudiante            & Parcial & No      & Sí      & Sí      \\
%		\midrule
%		Vista agregada del grupo                        & Parcial & No      & No      & Sí      \\
%		\midrule
%		Alertas y advertencias en tiempo real           & No      & No      & No      & Sí      \\
%		\midrule
%		Adaptación dinámica del contenido               & No      & Sí      & No      & Parcial \\
%		\midrule
%		Independencia del juego o plataforma            & No      & No      & No      & Sí      \\
%		\midrule
%		Usabilidad para docentes sin perfil técnico     & Sí      & Sí      & Sí      & Parcial \\
%		\midrule
%		Exportación o reutilización de datos            & No      & No      & No      & Sí      \\
%		\midrule
%		Integración con plataformas LMS                 & No      & No      & No      & Parcial \\
%		\bottomrule
%	\end{tabular}
%\end{table}
...
\begin{table}[H]
	...
	\label{tab:limitaciones_lad}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.0cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}>{\centering\arraybackslash}p{2.2cm}@{}}
			\toprule
			\textbf{Plataforma}
			& \textbf{Métricas aprendizaje profundo}
			& \textbf{Intervención proactiva docente}
			& \textbf{Vista individualizada}
			& \textbf{Vista agregada grupo}
			& \textbf{Alertas tiempo real}
			& \textbf{Adaptación dinámica}
			& \textbf{Independencia plataforma}
			& \textbf{Usabilidad docentes}
			& \textbf{Exportación datos}
			& \textbf{Integración LMS} \\
			\midrule
			\textbf{Kahoot!}           & No      & No      & Parcial & Parcial & No      & No      & No      & Sí      & No      & No      \\
			\midrule
			\textbf{Physics Playground} & Sí      & No      & Sí      & No      & No      & No      & No      & Sí      & No      & No      \\
			\midrule
			\textbf{RAGE Analytics}    & Sí      & Sí      & Sí      & Sí      & Sí      & Parcial & Sí      & Parcial & Sí      & Parcial \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
...
El presente capítulo concluye la fundamentación teórica de la plataforma educativa \textit{Hello World}, estableciendo el marco conceptual que sustenta las decisiones de diseño descritas en los capítulos posteriores. Los resultados del análisis permiten establecer las siguientes conclusiones:

\begin{itemize}
	\item Los videojuegos tienen la capacidad para construir conocimiento a través de la interactividad con los jugadores, los convierte en un formato idóneo para contextualizar contenidos de aprendizaje dentro de una historia significativa
	
	\item Los videojuegos logran materializar conceptos abstractos, transformando el error en una parte natural del aprendizaje, sin frustación, permitiendo que el estudiante avance a su propio ritmo
	
	\item El valor de la gamificación reside en su capacidad de lograr que el estudiante encuentre valor y disfrute en el propio proceso de aprendizaje
	
	\item Los videojuegos educativos adaptativos representan una evolución frente a los entornos estáticos. La integración de un modelo del estudiante y el ciclo continuo de adaptación permite calibrar el nivel de reto en tiempo real, evitando la frustación.
	
	\item Los tableros de analíticas de aprendizaje son el componente que cierra el ciclo de retroalimentación, sin embargo la mera visualización de datos o una alta complejidad algorítmica no garantizan una intervención pedagógica efectiva; es necesario que estos tableros sean accionables, proactivos y comprensibles con un solo vistazo del profesor.
